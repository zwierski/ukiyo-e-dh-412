...
if __name__ == "__main__":
    ...
    data['Description'] = data['Description'].apply(lambda x: '' if isinstance(x,float) else x)
    # remove the non-alphabetic chars
    data['Description'] = data['Description'].apply(remove_non_letters)
    ...
    text_prompts = ['Emperor', 'Shogun', 'Samurai', 'Minister', 'Constitution',
              ...
              'Mountain', 'Sea', 'Lake', 'Plant', 'Animal', 'Man', 'Woman','NaN']
    ...
    results = pd.DataFrame(columns=['image_path','text','top_probs','top_labels','label_index'])
